...
void TCPReceiver::receive( TCPSenderMessage message, Reassembler& reassembler, Writer& inbound_stream )
{
  ...
  if (ISN.has_value()){
    ...
    long int delta ;
    delta= ackno.has_value()? AbSlSeqno - ackno.value().unwrap(ISN.value(),preNeedIndex) - message.SYN : 0; // 这个是有问题的
    ...
    ackno = ISN.value() + nextNeedIndex + (this->SYN); // 修改成这个的理由
    if (FIN){
